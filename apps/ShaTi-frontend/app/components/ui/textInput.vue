<template>
  <div :class="disabled ? '' : 'hover:bg-primary-300'" class="my-2 inline-flex relative text-4xl font-bold transition duration-200 rounded-sm justify-center">
    ...
    <input type="text" class="absolute top-0 left-0 box-border w-full px-5 py-4 focus:outline-none underline decoration-transparent underline-offset-4 decoration-2 focus-visible:decoration-neutral-800 transition-[text-decoration-color] duration-200 whitespace-pre" @input="onInput" :disabled="disabled" :placeholder="placeholder" :value="modelValue" />
  </div>
</template>
